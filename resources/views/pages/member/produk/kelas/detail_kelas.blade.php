            ...
            <div class="row justify-content-between mt-5">
                <div class="col-11 mx-auto col-sm-7 mb-5 mb-sm-0">
                    ...
                    <div class="container mt-5">
                            ...
                            <div id="accordion" class="accordion mb-0">
                                ...
                                <div class="card shadow-xss mb-0 w-100">
                                    ...
                                    <div id="collapse-{{$bab['id']}}" class="collapse p-3 show" aria-labelledby="headingOne" data-parent="#accordion">
                                        ...
                                        <a href="{{route('enrollMateriKelas',['id'=>$data->id, 'ids'=>$bab->id])}}" class="card-body d-flex p-2 align-items-center">
                        ...
                        <h2 class="fw-700 font-sm mb-3 mt-1 pl-1 mb-3">{{$ujian->nama}}</h2>
                        <p class="font-xssss">Tersedia {{date_format(date_create($ujian->tanggal_mulai),"d M Y")}} sampai {{date_format(date_create($ujian->tanggal_selesai),"d M Y")}}</p>
                        ...
                        @if( now() > $ujian->tanggal_mulai && now() < $ujian->tanggal_selesai )
                        <a href="{{route('memberAttemptTest',$ujian->id)}}" class="btn border-0 btn-primary fw-600 font-xssss">Ambil ujian</a>
                        @else
                        <button class="btn border-0 btn-primary fw-600 font-xssss" disabled>Ambil ujian</button>
                        @endif
                    </div>
                </div>
                ...
                <div class="col-11 mx-auto col-sm-4">
                    ...
                    <section>
                        <div class="card d-block border-0 rounded-lg overflow-hidden p-4 shadow-xss mt-4">
                            <h2 class="fw-700 font-xs mb-3 mt-1 pl-1 mb-2">Nilai</h2>
                            <h4 class="font-sm fw-700 text-grey-600 mb-2 pl-2 position-relative lh-24">
                                {{$nilai}} <small>/ 100</small>
                            </h4>
                            <a href="{{route('memberDetailUjian',$ujian->id)}}">detail</a>
                        </div>
                    </section> 
                    @endisset
